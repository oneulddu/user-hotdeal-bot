...
- 원본 v2.3.2까지 통합하고 DB/API·알림 큐 보호·크롤러별 요청 설정 유지
- 공유 HTTP 클라이언트의 헤더 대소문자 처리, 응답 쿠키 격리 및 연결 풀 대기를 포함한 시간 제한 보강
- 따옴표·대소문자가 다른 charset 선언 지원 및 명시적 프록시와 환경변수의 우선순위 보존
- curl 요청 실패·취소 시 사용 중인 다른 요청을 보존하면서 연결 풀 복구
- 뽐뿌 작성자 장식 이미지 fallback 및 다모앙 쿼리·fragment 포함 URL 파싱 보강
- 피드 생성에서 불필요한 전체 건수 조회를 제거하고, 동일 조건의 동시 요청이 조회와 XML 생성을 공유하도록 개선
- 게시글 저장·삭제를 제한된 크기의 SQL 배치로 처리하고, 목록 조회 필터와 DB별 upsert 로직의 중복 정리
- DB 세션 팩터리 캐시의 엔진 참조 누수 수정 및 기본 세션의 공용 엔진 재사용
- 재로드·덤프 실패 시 대기 알림 큐 보존, 취소된 알림의 선두 복귀 비용 개선
- 종료·재로드를 진행 중인 크롤링 주기와 직렬화하고 Compose 크롤러 종료 유예시간을 2분으로 설정
- 아카라이브 V2의 중복 브라우저 초기화 방지 및 실패·취소 시 자원 정리
- Scrapling의 초기화·페이지 대기·재시도를 포함한 전체 요청에 기본 100초 상한 적용 (`ARCALIVE_SCRAPLING_TOTAL_TIMEOUT`으로 변경 가능)
- 아카라이브 V15의 HTTP 연결 재사용 및 요청 간 응답 쿠키 격리
- 설정 재로드에서 비활성화·교체될 기존 봇의 전송 차단 및 curl 옵션 오류 후 연결 풀 복구
- 크롤링 목록에서 일시적으로 사라진 최신 게시글이 다시 나타날 때 알림이 중복 전송되는 문제 수정
- 일시적으로 큰 게시글 ID가 관측된 뒤 그보다 작은 정상 신규 글의 알림이 계속 누락되는 문제 수정
